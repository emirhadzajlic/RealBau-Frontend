...
fetch(/*"https://realbauback.herokuapp.com/auth"*/"http://localhost:8080/auth", {
  ...
  .then(data => {
    ...
    if(data.isAuth) {
      if(window.location.href.indexOf("admin.html") > -1) {
      ...
      } else if(window.location.href.indexOf("insertExcel.html") > -1){
        if(data.role != "cc"){
          if(data.role == "ad") goToPage("admin")
          else goToPage("index")
        }
        ...
        toPage(data);
      } else if(window.location.href.indexOf("manageAcc.html") > -1){
        document.querySelector('#fname').value = data.ime + " " + data.prezime
      ...
      if(data.role !== "cc" && data.role !== "ad"){
        ...
        if(radiosWindow != null) var newContainer = radiosWindow.cloneNode(true)
        let node;
        let container = false;
        let nodeCount = 0;
        data.columns.split(", ").forEach(e => {
          if(e.toLowerCase().indexOf('finish') > -1) {
            if(nodeCount == 2 && radiosWindow != null){
            ...
            if(container && container !== node.parentElement){
              ...
              container.remove();
            } else if(container && container == node.parentElement){
              container.remove();
            }
            if(node !== null) {
      ...
      if(data.role !== "cc"){
        var toDelete = document.querySelectorAll('.cc-only');
        if(toDelete !== null) {
          for(var i = 0; i < toDelete.length; i++){
            toDelete[i].remove()
          }
      ...
      if(tableHead !== null){
        ...
        data.columns.split(", ").forEach(e => {
          ...
          if(e == "ORDERID"){
            node.classList.add("dont-show");
          }
          if(data.role === "mia"){
            if(e === "MDATUM" || e === "MTERMIN" || e === "ADATUM" || e === "AKTIVIRUNGTERMIN"){
              node.classList.add("dont-show");
            }
          }
          if(data.role !== "cc" && data.role !=="ad"){
            ...
            if(e == "STREET" || e == "HAUSNUMMER" || e == "ZUSAT" || e == "CITY" || e == "DP"){
              node.classList.add("for-phone");
            }
          }
      ...
      if(data.role == "ad"){
        document.querySelector('.userDataMenu').children[1].innerHTML = "ADMIN";
      } else if (data.role == "cc") {
        document.querySelector('.userDataMenu').children[1].innerHTML = "CALLCENTER";
      } else {
        document.querySelector('.userDataMenu').children[1].innerHTML = data.role.toUpperCase();
      }
    } else {
